        ...
        {isOpen && (
          <>
            ...
            <motion.div
              ...
              className="fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-[90vw] max-w-4xl h-[80vh] max-h-[700px] z-[9999]"
              style={{ zIndex: 9999 }}
            >
              {/* Close Button */}
              <button
                onClick={() => setIsOpen(false)}
                className="absolute -top-4 -right-4 w-8 h-8 bg-white rounded-full shadow-lg flex items-center justify-center text-gray-600 hover:text-gray-800 transition-colors z-[10000]"
                style={{ zIndex: 10000 }}
              >
                <X className="w-5 h-5" />
              </button>
